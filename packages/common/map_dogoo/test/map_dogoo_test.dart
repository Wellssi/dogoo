...
void main() {
  group('📌 MapDogoo - Function Test', () {
    test('👉 전달하는 값이 null이 아니면 Map에 추가한다', () {
      ...
      expect(myMap, <String, dynamic>{'key1': 1});

      mapSetIfPresent(map: myMap, key: 'key2', value: 2);

      expect(myMap, <String, dynamic>{'key1': 1, 'key2': 2});

      mapSetIfPresent(map: myMap, key: 'key3', value: null, defaultValue: 3);

      expect(myMap, <String, dynamic>{'key1': 1, 'key2': 2, 'key3': 3});
    });
    ...
    test('👉 전달하는 Map에 대해 보기 좋은 JSON String으로 반환', () {
      ...
      expect(
        mapToPrettyJsonString(myMap, indentSize: 4),
        '{\n    "key1": 1,\n    "key2": 2\n}',
      );
    });
  });
  ...
  group('📌 MapDogoo - Extension Test', () {
    ...
    test('👉 전달하는 Map에 대해 보기 좋은 JSON String으로 반환', () {
      ...
      expect(
        myMap.toPrettyJsonString(indentSize: 4),
        '{\n    "key1": 1,\n    "key2": 2\n}',
      );
    });
  });
